...
def render_resultados(df_g_raw: pd.DataFrame, df_d_raw: pd.DataFrame, feriado_atual: str, data_ancora_ida: str, data_ancora_volta: str):
    ...
    # feriado_atual = 'julho_2026' → já existe como coluna nos CSVs; renomeia para 'atual'
    ...
    for df_t in [df_g, df_d]:
        if 'metrica' in df_t.columns:
            df_t['metrica'] = (df_t['metrica']
                               .str.replace(r' \(Capacidade x Km\)', '', regex=True)
                               .str.replace(r' \(Pax x Km\)', '', regex=True))
    ...
    def format_kpi(metrica, valor):
        ...
        if "Ticket Médio" in metrica or "GMV" in metrica:
    ...
    with col_f1:
        ...
        filtro_data = st.selectbox(
            ...
            format_func=lambda x: pd.to_datetime(x).strftime('%d/%m/%Y') if x != "Consolidado Mês" else x,
    ...
    df_view = df_g.copy() if filtro_data == "Consolidado Mês" else df_d[df_d['data'] == filtro_data].copy()
    ...
    for i, row in enumerate(df_view.to_dict('records')):
        ...
        cols[i % 5].metric(
            label=nome,
            value=format_kpi(nome, row.get('atual')),
            delta=format_delta(row.get('atual'), row.get(ref_sel))
        )
    ...
    df_tg[f'Ref ({ref_nome})']= df_tg.apply(lambda r: format_kpi(r['Métrica'], r.get(ref_sel, 0)), axis=1)
    df_tg['Atual']            = df_tg.apply(lambda r: format_kpi(r['Métrica'], r.get('atual', 0)), axis=1)
    df_tg['Var %']            = df_tg.apply(lambda r: format_delta(r.get('atual', 0), r.get(ref_sel, 0)), axis=1)
